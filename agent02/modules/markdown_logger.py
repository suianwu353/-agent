...
def delete_task_from_markdown(task_content, time_iso, filename="task_log.md"):
    """
    根据任务内容和时间删除Markdown中的任务（分块格式，时间只比对到分钟）
    """
    try:
        if not os.path.exists(filename):
            return
        ...
        i = 0
        # 只比对到分钟，避免秒数差异
        time_prefix = time_iso[:16]  # '2025-09-18T14:00'
        while i < len(lines):
            line = lines[i]
            # 找到任务内容行
            if line.strip() == task_content:
                # 检查下方是否有时间行且匹配到分钟
                if i + 1 < len(lines) and lines[i + 1].strip().startswith("时间："):
                    time_line = lines[i + 1].strip()
                    # 支持时间行有秒或无秒
                    if time_line.startswith("时间："):
                        t_val = time_line.replace("时间：", "").strip()
                        if t_val.startswith(time_prefix):
                            # 跳过整个任务块（内容、时间、地点、天气、状态、空行）
                            k = i
                            while k < len(lines):
                                next_line = lines[k].strip()
                                # 到下一个任务内容、日期标题或文件结尾为止
                                if next_line == "" or next_line.startswith("状态："):
                                    k += 1
                                    break
                                k += 1
                            i = k
